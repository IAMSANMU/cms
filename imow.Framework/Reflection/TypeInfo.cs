...
namespace Imow.Framework.Reflection
{
    ...
    public sealed class TypeInfo
    {
        ...
        internal TypeInfo(System.Type type)
        {
            ...
            {
                IsNullable = true;
                NullableUnderlyingType = TypImowelper.GetTypeInfo(valueType);
                type = valueType;
            }
            ...
            {
                IsNumberType = true;
                EnumUnderlyingType = TypImowelper.GetTypeInfo(Enum.GetUnderlyingType(Type));
            }
            else if (IsNullable)
            {
        ...
        private TypeCodes _typeCodes = (TypeCodes)(-1);
        /// <summary> 获取类型的拓展枚举
        /// </summary>
            ...
            get
            {
                ...
                lock (this)
                {
                    if (IsMakeGenericType && _genericArgumentsTypeInfo == null)
                    {
                        ...
                        {
                            _genericArgumentsTypeInfo[i] = TypImowelper.GetTypeInfo(args[i]);
                        }
                    }
                }
                return _genericArgumentsTypeInfo;
