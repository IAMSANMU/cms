...
namespace Imow.Framework.Cache.DistributedCache.Storage.SSDB.NetSdk
{
	public class Client {
		public Link link;
        ...
        public void setx(string key, byte[] val, int expireseconds)
        {
            List<byte[]> resp = request("setx", _bytes(key), val, _bytes(expireseconds.ToString()));
            resp_code = _string(resp[0]);
            this.assert_ok();
        }
