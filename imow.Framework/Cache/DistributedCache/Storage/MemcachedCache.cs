...
namespace Imow.Framework.Cache.DistributedCache.Storage
{
    public class MemcachedCache : BaseCache
    {
        ...
        public override T GetValue<T>()
        {
            ...
            return Catch(() =>
            {
                ...
                try
                {
                    json = CompressFactory.DecompressString(this.Config.Compress, json);
        ...
        public override bool SetValue<T>(T value, TimeSpan expiretime)
        {
            return Catch(() =>
            {
                ...
                try
                {
                    ...
                    json = CompressFactory.CompressString(this.Config.Compress, json);
                }
                catch (Exception exp)
                {
        ...
        public override bool SetValue<T>(T value)
        {
            return Catch(() =>
            {
                ...
                var r = client.Store(Enyim.Caching.Memcached.StoreMode.Set, _key, json);
                if (r == false)
                    throw new DistributedCacheConnectException("执行SetValue(Store)函数失败");
                return r;
            });
        ...
        public override void Init()
        {
            Catch(() =>
            {
                var serverconfig = this.Config as MemcachedCacheConfig;
                client = new MemcachedClientPoolProvider().GetPool(serverconfig);
                return true;
            });
        }
    ...
    public class MemcachedCacheConfig : BaseConfig
    {
        /// <summary>
        /// 阿里云实例ID
        /// </summary>
