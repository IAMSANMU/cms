...
{
    class DapperOracle : DapperHelper
    {
        public DapperOracle(string connectionstring)
        {
        ...
        public override T GetById<T>(dynamic primaryId)
        {
            ...
            {
                conn.Open();
                return conn.Get<T>(primaryId as object, dbType: DbType.ORACLE);
            }
        }
        /// <summary>
        /// 根据多个Id获取多个实体
        ...
        public override IEnumerable<T> GetAll<T>()
        {
            ...
            {
                conn.Open();
                return conn.GetList<T>(dbType: DbType.ORACLE);
            }
        ...
        public override int Count<T>(object predicate, bool buffered = false)
        {
            ...
            {
                conn.Open();
                return conn.Count<T>(predicate, dbType: DbType.ORACLE);
            }
        ...
        public override IEnumerable<T> GetList<T>(object predicate = null, IList<ISort> sort = null, bool buffered = false)
        {
            ...
            {
                conn.Open();
                return conn.GetList<T>(predicate, sort, null, null, buffered, dbType: DbType.ORACLE);
            }
        }
        /// <summary>
        /// 单表分页
            ...
            IList<ISort> sort = null, bool buffered = true)
        {
            using (OracleConnection conn = new OracleConnection(ConnectionString))
            {
                ...
                IEnumerable<T> entityList = conn.GetPage<T>(predicate, sort, pageIndex, pageSize, null, null,
                    buffered, dbType: DbType.ORACLE);
                allRowsCount = conn.Count<T>(predicate, dbType: DbType.ORACLE);
                return entityList;
            }
        }
        ...
        public override IEnumerable<TReturn> GetPageList<TFirst, TSecond, TReturn>(int pageIndex, int pageSize, out long allRowsCount, Func<TFirst, TSecond, TReturn> map,  IPredicate predicate = null, IList<ISort> sort = null, JoinType joinType = JoinType.Inner, string splitOn = "id", bool buffered = true)
        {
            using (OracleConnection conn = new OracleConnection(ConnectionString))
            {
                ...
                IEnumerable<TReturn> entityList = conn.GetPage(map, joinType, splitOn, predicate, sort, pageIndex, pageSize, null, null,
                    buffered, dbType: DbType.ORACLE);
                allRowsCount = conn.Count<TFirst, TSecond>(joinType, predicate, dbType: DbType.ORACLE);
                return entityList;
            }
        }
        /// <summary>
        ...
        public override dynamic Insert<T>(T entity)
        {
            ...
            {
                result = Framework.Db.Transaction.Current.DbTransactionWrapper.DbTransaction.Connection.Insert(entity, transaction: Imow.Framework.Db.Transaction.Current.DbTransactionWrapper.DbTransaction, dbType: DbType.ORACLE);
            }
            else
            {
                using (OracleConnection conn = new OracleConnection(ConnectionString))
                {
                    conn.Open();
                    result = conn.Insert(entity, transaction: null, dbType: DbType.ORACLE);
                }
            }
            return result;
        }
        ...
        public override bool Update<T>(T entity)
        {
            ...
            if (null != Imow.Framework.Db.Transaction.Current)
            {
                isOk = Framework.Db.Transaction.Current.DbTransactionWrapper.DbTransaction.Connection.Update(entity, Imow.Framework.Db.Transaction.Current.DbTransactionWrapper.DbTransaction, dbType: DbType.ORACLE);
            }
            else
            {
                using (OracleConnection conn = new OracleConnection(ConnectionString))
                {
                    conn.Open();
                    isOk = conn.Update(entity, null, dbType: DbType.ORACLE);
                }
            }
            return isOk;
        }
        ...
        public override int Delete<T>(dynamic primaryId)
        {
            ...
            if (null != Imow.Framework.Db.Transaction.Current)
            {
                isOk = Framework.Db.Transaction.Current.DbTransactionWrapper.DbTransaction.Connection.Delete(obj, Imow.Framework.Db.Transaction.Current.DbTransactionWrapper.DbTransaction, dbType: DbType.ORACLE);
            }
            else
            {
                using (OracleConnection conn = new OracleConnection(ConnectionString))
                {
                    conn.Open();
                    isOk = conn.Delete(obj, null, dbType: DbType.ORACLE);
                }
            }
            return isOk;
        }
        ...
        public override int DeleteList<T>(object predicate = null)
        {
            ...
            if (null != Imow.Framework.Db.Transaction.Current)
            {
                delNum = Framework.Db.Transaction.Current.DbTransactionWrapper.DbTransaction.Connection.Delete<T>(predicate, Imow.Framework.Db.Transaction.Current.DbTransactionWrapper.DbTransaction, dbType: DbType.ORACLE);
            }
            {
                using (OracleConnection conn = new OracleConnection(ConnectionString))
                {
                    conn.Open();
                    delNum = conn.Delete<T>(predicate, null, dbType: DbType.ORACLE);
                }
            }
            return delNum;
        }
        ...
        public override IEnumerable<TReturn> GetPageList<TFirst, TSecond, TReturn>(string sql, int pageIndex, int pageSize, out long allRowsCount, Func<TFirst, TSecond, TReturn> map, IPredicate predicate = null, IList<ISort> sort = null, string splitOn = "id", bool buffered = true)
        {
            using (OracleConnection conn = new OracleConnection(ConnectionString))
            {
                var entityList = conn.GetPage(sql, map, splitOn, predicate, sort, pageIndex, pageSize, out allRowsCount, null, null, buffered, dbType: DbType.ORACLE);
                return entityList;
            }
        }
        /// <summary>
