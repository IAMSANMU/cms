...
namespace DapperExtensions.Sql
{
    public class SqlServerDialect : SqlDialectBase
    {
        ...
        public override string GetMultipleJoinSql(List<string> tablenames, List<string> resultList, string joinStr)
        {
            string result = null;
            ...
            for (int i = 0; i < tablenames.Count; i++)
            {
                ...
                {
                    sqlBuilder.Append(" "+ joinStr + " "+tablename+" on {"+(i-1)+"} ");
                }
            }
        
                result = string.Format(sqlBuilder.ToString(), resultList.ToArray());
            return result;
        }
    }
}